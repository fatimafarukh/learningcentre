	...
	if(isset($_REQUEST['submit']) and check_admin_referer('watu_questions')) {
		...
		if($action == 'edit'){ //Update goes here
			...
			$wpdb->query($wpdb->prepare("UPDATE ".WATU_QUESTIONS." 
				SET question=%s, answer_type=%s, is_required=%d, feedback=%s, is_inactive=%d, is_survey=%d
				WHERE ID=%d", $content, $answer_type, $is_required, 
				$feedback, $is_inactive, $is_survey, $question_id));
			...
			$sql = $wpdb->prepare("INSERT INTO ".WATU_QUESTIONS." (exam_id, question, answer_type, is_required, feedback, sort_order, is_inactive, is_survey) 
			VALUES(%d, %s, %s, %d, %s, %d, %d, %d)", $quiz_id, $content, $answer_type, 
				$is_required, $feedback, $sort_order, $is_inactive, $is_survey);
			$wpdb->query($sql);
